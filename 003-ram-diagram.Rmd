...
\begin{tikzpicture}[
  ...
  \draw[cov] (eta_1) to[out=70,in=110,looseness=5] node[above] {$\psi_{\eta_1}$} (eta_1);
  \draw[cov] (eta_2) to[out=-70,in=-110,looseness=5] node[below] {$\psi_{\eta_2}$} (eta_2);
...
```{tikz, ram-diagram-05, echo = FALSE, fig.cap = "Path Model with Latent Variables", fig.ext=if (knitr:::is_latex_output()) "pdf" else "png", cache = TRUE}
\usetikzlibrary{er, arrows, positioning}
\begin{tikzpicture}[
  auto,
  node distance = 15mm,
