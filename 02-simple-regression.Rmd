...
```{tikz, regression-01, echo = FALSE, fig.width = 4, fig.cap = "The Simple Linear Regression Model (with $v_3$)", fig.ext=if (knitr:::is_latex_output()) "pdf" else "png", cache = TRUE}
...
\begin{tikzpicture}[
  ...
  \node[error] (v3) [below = of v1] {$v_3$};
  \draw [path] (1) to node {$m_1$} (v1);
  \draw [path] (1) to node {$m_2$} (v2);
  \draw [path] (v2) to node {$a_{1,2}$} (v1);
  \draw [path] (v3) to node {1} (v1);
  \draw [cov] (v3) to[out=-60,in=-120,looseness=7] node[below] {$\omega_{3, 3}$} (v3);
...
```{tikz, regression-02, echo = FALSE, fig.width = 4, fig.cap = "The Simple Linear Regression Model (without $v_3$)", fig.ext = "png", cache = TRUE}
\usetikzlibrary{er, arrows, positioning}
\begin{tikzpicture}[
  auto,
  node distance = 20mm,
...
Random data from the multivariate normal distribution using the Reticular Action Model (RAM) notation
can be generated using the `ramR::mvn()` function.
...
knitr::kable(
  muhat,
  col.names = "$\\boldsymbol{\\hat{\\mu}}$",
  caption = "$\\boldsymbol{\\hat{\\mu}}$",
  escape = FALSE
)
```
